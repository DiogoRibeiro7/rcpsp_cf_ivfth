...
class TestModelValidation:
    ...
    def test_unknown_predecessor(self, toy_instance):
        ...
        from rcpsp_cf_ivfth import NIVTF, Activity, ModeData, create_triangle
    ...
    def test_calendar_period_validation(self, toy_instance):
        ...
        bad_calendar = CalendarParams(
            T_days=30, Y_periods=[(1, 30), (31, 60)]  # Too small  # Goes up to day 60
        )
    ...
    def test_empty_periods_validation(self, toy_instance):
        ...
        bad_calendar = CalendarParams(T_days=60, Y_periods=[])  # Empty

        with pytest.raises(
            ValueError, match="At least one long period \\(Y_periods\\) is required"
        ):
            RCPSP_CF_IVFTH(activities, finance, bad_calendar)
    ...
    def test_alpha_level_propagation(self, toy_instance, ivfth_weights):
        ...
        for alpha in [0.0, 0.3, 0.7, 1.0]:
            targets = IVFTHTargets(
                alpha_level=alpha, Z1_PIS=10.0, Z1_NIS=60.0, Z2_PIS=30000.0, Z2_NIS=0.0
            )
