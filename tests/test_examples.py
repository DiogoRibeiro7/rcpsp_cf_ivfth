...
from rcpsp_cf_ivfth import NIVTF, Activity, CalendarParams, FinanceParams
    ...
    def test_toy_instance_resource_structure(self):
        ...
        for activity in activities.values():
            for mode in activity.modes.values():
                ...
                for res_id in mode.nonrenewables.keys():
                    assert res_id in finance.CW_l, f"Resource {res_id} not in cost dictionary"
    ...
    def test_toy_instance_fuzzy_numbers(self):
        ...
        for activity in activities.values():
            for mode in activity.modes.values():
                ...
                for nivtf in mode.renewables.values():
                ...
                for nivtf in mode.nonrenewables.values():
                    assert isinstance(nivtf, NIVTF)
                    assert nivtf.E1_L() >= 0
                    assert nivtf.E2_L() >= 0
...
class TestExampleVariations:
    ...
    def test_modified_toy_instance(self):
        ...
        targets = IVFTHTargets(
            alpha_level=0.3, Z1_PIS=15.0, Z1_NIS=90.0, Z2_PIS=40000.0, Z2_NIS=0.0
        )
    ...
    def test_minimal_instance(self):
        ...
        from rcpsp_cf_ivfth import Activity, CalendarParams, FinanceParams, ModeData
