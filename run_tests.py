...
KNOWN_SOLVERS = ("cbc", "glpk", "appsi_highs", "gurobi", "cplex")
...
def run_tests(args):
    ...
    if args.no_solver:
        # Solver-backed tests cover most of model.py, so the project-wide coverage
        # gate cannot be met once they are deselected.
        cmd.extend(["-m", "not solver", "--cov-fail-under=0"])
    ...
    if available_solvers:
        ...
        print("Install HiGHS (wheel, all platforms): pip install highspy")
...
Examples:
  ...
  python run_tests.py --no-solver        # Skip solver-dependent tests
    ...
    parser.add_argument(
        "-n", "--parallel", type=int, metavar="N", help="Run tests in parallel with N workers"
    )

    parser.add_argument("--fast", action="store_true", help="Skip slow tests")

    parser.add_argument(
        "--no-solver", action="store_true", help="Skip tests that require MILP solvers"
    )

    parser.add_argument(
        "-k", "--pattern", metavar="PATTERN", help="Run only tests matching the given pattern"
    )

    parser.add_argument(
        "--pytest-args", metavar="ARGS", help="Additional arguments to pass to pytest"
    )
